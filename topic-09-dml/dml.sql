...
-- Унікальний частковий індекс uq_borrowing_copy_active гарантує, що один
-- примірник не може бути одночасно у двох активних (return_date IS NULL) видачах.
-- Тут кожен примірник 1..10 фігурує лише раз, тому обмеження дотримано.
...
VALUES
  (1, 1,  NULL),   -- примірник ще не призначений
  (1, 2,  NULL),
  (2, 3,  NULL),
  (3, 4,  7),      -- примірник 7 належить книзі 4 і вільний (повернений)
  (4, 5,  NULL),   -- обидва примірники книги 5 (9,10) в активних видачах
  (5, 6,  NULL),   -- примірників книги 6 у наборі немає
  (6, 7,  NULL),
  (7, 8,  NULL),   -- примірників книги 8 у наборі немає
  (8, 9,  NULL),
  (9, 10, NULL);   -- примірників книги 10 у наборі немає
...
-- ################################################################
-- ПРИМІТКА:
-- 1) НЕМАЄ UPDATE ДЛЯ ТАБЛИЦЬ ЗВ'ЯЗКУ (many-to-many).
--    Робити UPDATE у таблицях багатьох-до-багатьох немає сенсу з боку
--    бізнес-логіки: зв'язок або існує (INSERT), або ні (DELETE).
-- 2) borrowing_items не оновлюється: дата видачі та початковий термін
--    (due_date) фіксуються при оформленні і не змінюються; повернення
--    моделюється окремим UPDATE return_date у реальній системі.
-- 3) borrowings не оновлюється напряму у цьому наборі: зміна статусу видачі
--    у реальній системі залежить від стану borrowing_items.
...
-- ----------------------------------------------------------------
-- Krysa Oleksandr: categories / book_categories / book_authors
-- ----------------------------------------------------------------
...
-- примірник BC-0011 (id=11) навмисно не використовується у
-- borrowing_items та reservation_items, тому FK ON DELETE RESTRICT не блокує видалення.
DELETE FROM "book_copies"
WHERE "barcode" = 'BC-0011';

-- members: видалення читача без активних видач та резервувань.
-- читач 'removable' (id=11) навмисно не має записів у borrowings та
-- reservations (FK ON DELETE RESTRICT заблокував би видалення). Його відгуки у
-- book_reviews (якщо є) видаляються автоматично через FK ON DELETE CASCADE.
DELETE FROM "members"
...
-- книзі 1 належать примірники 1 і 2 .
-- примірник 1 (повернений у видачі 1, тому вільний для призначення).
